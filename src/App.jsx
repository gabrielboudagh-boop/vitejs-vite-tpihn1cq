...
<!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets>
  <x:ExcelWorksheet><x:Name>Questions</x:Name><x:WorksheetOptions><x:Selected/></x:WorksheetOptions></x:ExcelWorksheet>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.72)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:500,padding:20}}>
      <div className="fade-in" style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:18,width:"100%",maxWidth:520,maxHeight:"92vh",overflowY:"auto",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{padding:"24px",flex:1}}>
          ...
          {s==="anki"&&(<><div style={{textAlign:"center",marginBottom:22}}><div style={{fontSize:34,marginBottom:8}}>⚡</div><div style={{fontSize:20,fontWeight:700,color:T.text,marginBottom:4}}>Flashcard</div><div style={{fontSize:13,color:T.muted}}>1-liner for your Anki deck</div></div><div style={{background:T.raised,border:`1px solid ${T.border}`,borderRadius:10,padding:"14px 16px",marginBottom:16}}><div style={{fontSize:10,color:T.muted,letterSpacing:"0.8px",marginBottom:8}}>✨ AI SUGGESTED FRONT</div>{ankiLoading?<div style={{color:T.muted,fontSize:12,fontStyle:"italic"}}>Generating...</div>:aiAnki?<><div style={{fontSize:13,color:T.text,lineHeight:1.6,marginBottom:10}}>{aiAnki}</div><button onClick={()=>set("ankiFront",aiAnki)} style={{background:T.accent,border:"none",borderRadius:6,padding:"5px 12px",color:"#fff",fontSize:11,cursor:"pointer"}}>Use this →</button></>:<div style={{fontSize:12,color:T.muted}}>Fill in concept above for AI suggestion.</div>}</div><Lbl T={T}>Your 1-liner (Anki front)</Lbl><Inp T={T} placeholder="e.g. 55M jaw claudication + vision loss → next step?" value={data.ankiFront} onChange={e=>set("ankiFront",e.target.value)}/><div style={{fontSize:11,color:T.muted,marginTop:8}}>This becomes the front of your Anki card.</div></>)}
...
// e.g. "https://vimavima-anki.railway.app"
...
export default function App(){
  ...
  const [mode,setMode]=useState("USMLE");
    ...
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"'DM Sans',sans-serif",paddingBottom:60}}>
      ...
      <div style={{padding:"18px 32px 14px",borderBottom:`1px solid ${T.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12,background:T.bg}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          <div style={{position:"relative"}}>
            <button onClick={()=>setShowModeMenu(m=>!m)} style={{background:T.raised,border:`1px solid ${T.border}`,borderRadius:8,padding:"7px 14px",fontSize:12,color:T.dim,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>{mode} ▾</button>
            {showModeMenu&&(<div style={{position:"absolute",top:"calc(100% + 6px)",right:0,background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,overflow:"hidden",zIndex:100,minWidth:140,boxShadow:"0 8px 30px rgba(0,0,0,0.25)"}}>
              {Object.keys(MODES).map(m=><button key={m} onClick={()=>{setMode(m);setShowModeMenu(false);setTab("sessions");setFilterSubject("All");}} style={{display:"block",width:"100%",background:m===mode?T.accent+"18":"none",border:"none",padding:"10px 16px",fontSize:13,color:m===mode?T.accent:T.dim,cursor:"pointer",textAlign:"left",fontWeight:m===mode?600:400}}>{MODES[m].label}</button>)}
            </div>)}
          </div>
      ...
      {/* Tabs */}
      <TabBar T={T} tabs={[["sessions","📁  Sessions"],["allquestions","≡  All Questions"],["analytics","📊  Analytics"],["flashcards",`⚡  Flashcards${ankiTotal>0?" ("+ankiTotal+")":""}`]]} active={tab} onChange={setTab} style={{padding:"0 32px"}}/>
      ...
      {showModeMenu&&<div style={{position:"fixed",inset:0,zIndex:99}} onClick={()=>setShowModeMenu(false)}/>}
    </div>
  );
}